...
bool VerifyCardNumber(long _CardNumber)
{
    ...
    while (LastTestedNumber + 2 <= LengthOfCN)
    {
        // This is the key part of the calculation, this is the first part of Luhn's algorithm, research on this law can be found here: https://cs50.stackexchange.com/questions/28830/luhn-algorithm-error-pset1 or https://cs50.harvard.edu/x/2022/psets/1/credit/#luhns-algorithm
        LastTestedNumber += 2;
...
int IdentifyLengthOfLong(long _CardNumber)
{
    // Calculates the length of the supplied number, this is done by dividing the number ans keeping track of the number of divisions and stops when the value is less than ten. Boom length is calculated.
    int Length = 1;
...
long PersonalFindXDigit(long _CardNumber,
                        int NumberFind) // I would comment this code but even I do not understand this monstrosity.
{
...
long OtherFindXDigit(long _CardNumber, int
                     NumberFind) // This is my code which I took inspiration from out of stack overflow, https://stackoverflow.com/questions/3389264/how-to-get-the-separate-digits-of-an-int-number
{
    ...
    } // Deeper explanation, imagine you have the number 1254 and want to single out the second digit(5), the code written in the loop takes 1254 and divides it by 10 and then floors that number, this leads to 125 and then if you modulus this by 10(125 % 10 = 5), boom, answer. modulus is like division, although where it differs is it provides the remainder not the answer.
    return _CardNumber % 10; // This code takes the cut down version of the previous line and singles out the correct digit.
}

void DetermineCardOwner(long
                        _CardNumber)
{
    // This function leads to others that find out weather the provided card number's details lead to that provider.
    long CardLength = IdentifyLengthOfLong(
                          _CardNumber); // Determines card length so that this function does not need to be repeated, leading to lower efficiency.
...
bool AMEXCheck(long _CardNumber, int CardLength)
{
    int FirstTwoDigits = OtherFindXDigit(_CardNumber,
                                         CardLength) * 10;
// This returns the last number in the card and creates a new variable the last number is the tens so is multiplied by ten.
    FirstTwoDigits += OtherFindXDigit(_CardNumber,
                                      CardLength - 1); // This returns the second last number in the card, being the ones, this is done using the same method as above.
    if (CardLength == 15)
    {
...
bool MasterCardCeck(long _CardNumber, int
                    CardLength)
{
    // This function is the same as the AMEX with slight changes for the card supplier.
